...
class SSOStrategy extends OpauthStrategy{
	...
	public function request(){
		$url = 'http://sso.tusur.ru/oauth/authorize/';
		$params = array(
	...
	public function oauth2callback(){
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
			$code = $_GET['code'];
			$url = 'http://sso.tusur.ru/oauth/token';
			$params = array(
				'code' => $code,
				'client_id' => $this->strategy['client_id'],
	...
	private function userinfo($access_token){
		$userinfo = $this->serverGet('http://sso.tusur.ru/oauth/user.json', array('oauth_token' => $access_token), null, $headers);
		if (!empty($userinfo)){
